...
// (From gramene-primers' scripts/lint-pkg.mjs.)
//
// `attw --pack .` runs `npm pack` in the package root and deletes the resulting
// gramene-atlas-heatmap-<version>.tgz afterwards, which is the same file `npm run pack:local` makes for
...
const dir = mkdtempSync(join(tmpdir(), 'gramene-atlas-heatmap-attw-'));
try {
  run(bin('publint'), []);
